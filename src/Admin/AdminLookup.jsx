...
//       if (res.ok) {
//         const data = await res.json();
//         // Filter plans to match exact application combination
...
//             selectedAppNames.every(name => planAppNames.includes(name))
//           );
//         });
...
//           onClose: () => navigate('/admin') // Redirect after toast closes
//         });
...
//     e.preventDefault();
//     if (!form.applicationIds.length || !form.planTypeMappingId || !form.monthlyBasePrice) {
...
//         body: JSON.stringify({
//           ...form,
//           monthlyBasePrice: parseFloat(form.monthlyBasePrice),
...
//         <h2 className="text-3xl font-bold text-gray-800 mb-6">Manage Subscription Plans</h2>
...
//                 <th className="p-3 text-left text-sm font-medium">Plan Type</th>
//                 <th className="p-3 text-left text-sm font-medium">Monthly Price</th>
//                 <th className="p-3 text-left text-sm font-medium">Calculated Price</th>
//                 <th className="p-3 text-left text-sm font-medium">Discount</th>
//                 <th className="p-3 text-left text-sm font-medium">Final Price</th>
//                 <th className="p-3 text-left text-sm font-medium">Stripe Price ID</th>
...
//                     <td className="p-3 text-sm text-gray-600">{appNames}</td>
//                     <td className="p-3 text-sm text-gray-600">{planType ? formatPlanType(planType) : `Plan ${plan.plan.id}`}</td>
//                     <td className="p-3 text-sm text-gray-600">${plan.monthlyBasePrice.toFixed(2)}</td>
//                     <td className="p-3 text-sm text-gray-600">${plan.calculatedPrice.toFixed(2)}</td>
//                     <td className="p-3 text-sm text-gray-600">{plan.discountPercent ? `${plan.discountPercent}%` : '0%'}</td>
//                     <td className="p-3 text-sm font-semibold text-gray-800">${plan.discountedPrice.toFixed(2)}</td>
...
//         {/* Stripe Sync Confirmation Modal */}
//         {isSyncModalOpen && (
//           <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
//             <div className="bg-white rounded-lg p-6 max-w-md w-full">
    ...
    try {
      ...
      if (res.ok) {
        ...
        const filteredPlans = data.filter(plan => {
          ...
          const planAppNames = plan.applications ? plan.applications.map(a => a.name).sort() : [];
          const selectedAppNames = [...appNames].sort();
          console.log('Plan App Names:', planAppNames, 'Selected App Names:', selectedAppNames); // Debug log
          return (
            planAppNames.length === selectedAppNames.length &&
            planAppNames.every(name => selectedAppNames.includes(name))
          );
          ...
          <table className="w-full border-collapse bg-white shadow-sm rounded-lg">
            <thead>
              <tr className="bg-gray-200 text-gray-700">
                <th className="p-3 text-left text-sm font-medium">Applications</th>
                <th className="p-3 text-left text-sm font-medium">Plan Type</th>
                <th className="p-3 text-left text-sm font-medium">Monthly Base Price</th>
                <th className="p-3 text-left text-sm font-medium">Calculated Price</th>
                <th className="p-3 text-left text-sm font-medium">Discount</th>
                <th className="p-3 text-left text-sm font-medium">Final Price</th>
                <th className="p-3 text-left text-sm font-medium">Stripe Price ID</th>
              ...
              {currentPlans.map((plan, index) => {
                ...
                return (
                  <tr key={plan.id} className={`${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'} hover:bg-gray-100 transition-colors`}>
                    <td className="p-3 text-sm text-gray-600">{appNames}</td>
                    <td className="p-3 text-sm text-gray-600">{planType ? formatPlanType(planType) : `Plan ${plan.plan.id}`}</td>
                    <td className="p-3 text-sm text-gray-600">${(plan.monthlyBasePrice ?? 0).toFixed(2)}</td>
                    <td className="p-3 text-sm text-gray-600">${(plan.calculatedPrice ?? 0).toFixed(2)}</td>
                    <td className="p-3 text-sm text-gray-600">{plan.discountPercent ? `${plan.discountPercent}%` : '0%'}</td>
                    <td className="p-3 text-sm font-semibold text-gray-800">${(plan.discountedPrice ?? 0).toFixed(2)}</td>
                    <td className="p-3 text-sm text-gray-600">
                      {plan.syncing ? (
                        <span className="text-gray-500">Syncing...</span>
                      ) : plan.stripePriceId ? (
