...
const ChangeSuccess = () => {
  ...
  useEffect(() => {
    const verifySession = async () => {
      if (!sessionId) {
        toast.error(t('changeSuccess.invalidSessionId'));
        setTimeout(() => navigate('/subscription-dashboard'), 2000);
        return;
      }
      ...
      try {
        ...
        const data = await response.json();
        if (!response.ok) {
          throw new Error(data.message || t('changeSuccess.failedToVerify'));
        }
        if (data.status === 'success') {
          setVerified(true);
          toast.success(t('changeSuccess.planChangedSuccess'));
        } else {
          throw new Error(data.message || t('changeSuccess.verificationFailed'));
        }
      } catch (err) {
        setError(`${t('changeSuccess.verificationFailed')}: ${err.message}`);
        toast.error(`${t('changeSuccess.error')}: ${err.message}`);
        setTimeout(() => navigate('/subscription-dashboard'), 2000);
      }
    };
      ...
      const timer = setInterval(() => {
        setCountdown((prev) => {
          if (prev <= 1) {
            clearInterval(timer);
            navigate('/subscription-dashboard', {
              state: { successMessage: t('changeSuccess.planChangedConfirmation') },
            });
            return 0;
          }
          return prev - 1;
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-100 to-blue-50" role="alert" aria-live="assertive">
        ...
        >
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <h2 className="text-2xl font-bold text-red-600">{t('changeSuccess.error')}</h2>
          </div>
          <p className="text-gray-600 mb-4">{error}</p>
          <p className="text-gray-600">
            {t('changeSuccess.redirectingIn')} <span className="font-bold">{countdown}</span> {t('changeSuccess.seconds')}
          </p>
        </motion.div>
      </div>
    );
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-100 to-blue-50" role="alert" aria-live="polite">
      ...
      >
        <div className="text-center">
          ...
          <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">
            {t('changeSuccess.subscriptionChanged')}
          </h2>
          <p className="text-gray-600 mb-4">
            {t('changeSuccess.planChangeScheduled')}
          </p>
          <div className="flex justify-center items-center space-x-3 mb-4">
            <Loader2 className="h-6 w-6 text-indigo-500 animate-spin" />
            <p className="text-gray-600">
