...
//           if (prev <= 1) {
//             clearInterval(timer);
//             navigate('/subscription-dashboard'); // Redirect to subscription dashboard
//             return 0;
//           }
//           return prev - 1;
//         });
...
//           </div>
//           <p className="text-gray-600">Verifying your product addition...</p>
//         </div>
//       </div>
//     );
//   }
...
//             Congratulations! Your new product has been successfully added to your subscription.
//           </p>
...
const AddSuccess = () => {
  ...
  useEffect(() => {
    const verifySession = async () => {
      if (!sessionId) {
        toast.error(t('addSuccess.invalidSessionId'));
        setTimeout(() => navigate('/subscription-dashboard'), 2000);
        return;
      }
      try {
        ...
        const data = await response.json();
        if (!response.ok) {
          throw new Error(data.message || t('addSuccess.failedToVerify'));
        }
        setVerified(true);
        toast.success(t('addSuccess.productAddedSuccess'));
      } catch (err) {
        toast.error(`${t('addSuccess.verificationFailed')}: ${err.message}`);
        setTimeout(() => navigate('/subscription-dashboard'), 2000);
      }
    };
    verifySession();
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-100 to-blue-50">
        <div className="bg-white p-6 md:p-8 rounded shadow-lg w-full max-w-md text-center">
          ...
          <p className="text-gray-600">{t('addSuccess.verifyingAddition')}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-100 to-blue-50">
      <div className="bg-white p-6 md:p-8 rounded shadow-lg transform transition-all duration-300 hover:shadow-xl w-full max-w-md">
        <div className="text-center">
          ...
          <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">
            {t('addSuccess.productAddedSuccessfully')}
          </h2>
          <p className="text-gray-600 mb-4">
            {t('addSuccess.congratulationsMessage')}
          </p>
          <div className="flex justify-center items-center space-x-3 mb-4">
            <div className="animate-spin rounded-full h-6 w-6 border-t-2 border-b-2 border-indigo-500"></div>
            <p className="text-gray-600">
              {t('addSuccess.redirectingIn')} <span className="font-bold">{countdown}</span> {t('addSuccess.seconds')}
            </p>
          </div>
        </div>
      </div>
