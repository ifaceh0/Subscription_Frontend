...
// update code for language support
import { useLocation, useNavigate } from 'react-router-dom';
import { useState, useEffect, useRef } from 'react';
import { toast } from 'react-toastify';
...
const Success = () => {
  ...
  const verifySession = async (silent = false) => {
    if (!sessionId) {
      setStatus('error');
      setErrorMessage(t('success.invalidSessionId'));
      if (!silent) toast.error(t('success.invalidSessionId'));
      return false;
    }
    ...
    try {
      ...
      if (response.ok && data.status === 'success') {
        setStatus('success');
        if (!silent) toast.success(t('success.subscriptionActivated'));
        return true;
      } else if (data.status === 'pending') {
        setStatus('processing');
        return false;
        ...
        setStatus('error');
        const msg = data.message || t('success.failedToVerify');
        setErrorMessage(msg);
        if (!silent) toast.error(msg);
        return false;
      }
      ...
      setStatus('error');
      setErrorMessage(t('success.networkError'));
      if (!silent) toast.error(t('success.verificationFailed'));
      return false;
    }
  };
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      if (attempts >= maxAttemptsRef.current) {
        clearInterval(pollIntervalRef.current);
        setStatus('timeout');
        toast.warn(t('success.takingLongerThanExpected'));
      }
    };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-red-50 to-gray-100">
        <div className="bg-white p-8 rounded shadow-xl max-w-md text-center">
          ...
          <h2 className="text-3xl font-bold text-gray-800 mb-4">{t('success.somethingWentWrong')}</h2>
          <p className="text-gray-600 mb-6">{errorMessage}</p>
          <button
            onClick={() => navigate('/subscription')}
            className="px-8 py-3 bg-indigo-600 text-white rounded hover:bg-indigo-700 transition"
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-yellow-50 to-orange-50">
        <div className="bg-white p-8 rounded shadow-xl max-w-md text-center">
          ...
          <h2 className="text-3xl font-bold text-gray-800 mb-4">{t('success.takingLonger')}</h2>
          <p className="text-gray-600 mb-6">
            {t('success.subscriptionBeingProcessed')}
          </p>
          <button
            onClick={() => navigate('/subscription')}
            className="px-8 py-3 bg-indigo-600 text-white rounded hover:bg-indigo-700 transition"
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-50 to-teal-50">
        <div className="bg-white p-8 rounded shadow-2xl max-w-md text-center transform transition-all hover:scale-105">
          ...
          <h2 className="text-4xl font-bold text-gray-800 mb-4">{t('success.success')}</h2>
          <p className="text-lg text-gray-600 mb-6">
            {t('success.subscriptionActivatedMessage')}
          </p>
          <div className="flex items-center justify-center space-x-3">
            <div className="animate-spin rounded-full h-6 w-6 border-t-2 border-b-2 border-indigo-500"></div>
            <p className="text-gray-700 font-medium">
              {t('success.redirectingIn')} <span className="font-bold text-indigo-600">{countdown}</span> {t('success.seconds')}
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-50 to-purple-50">
      <div className="bg-white p-10 rounded shadow-2xl max-w-md text-center">
        ...
        <h2 className="text-3xl font-bold text-gray-800 mb-4">{t('success.processingSubscription')}</h2>
